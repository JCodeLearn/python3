...
| ^ | 匹配字符串的开头 |
| $ | 匹配字符串的结尾 |
| \b | 匹配单词边界，假如需要匹配单词 Chapter，那么正则表达式 /\bCha/ 将匹配到 Chapter 这个单词的 Cha 部分。注意，单词与空格可以构成单词边界，但是单词与其他非单词字符也可构成单词边界，如 \> 。 |
...
| 非捕获分组 | (?:pattern) | 分组但不捕获，如模式 `(?:Mr\|Ms\|Mrs)\. (\w+)` 匹配 "Mr. Smith" 但只捕获 "Smith"。 |
| 命名分组 | (?P<name>pattern) (Python) | 为分组指定名称 |
| 反向引用（在正则表达式模式内） | \1, \2 等 | 引用前面匹配的分组 |
| 命名反向引用（在正则表达式模式内） | (?P=name) (Python) | 按名称引用分组 |
| 替换引用（在正则表达式模式外） | \1, \2 等 | 在替换字符串中引用分组 |
| 条件匹配 | 如：`(?(1)true-pattern\|false-pattern)` | 表示如果分组 1 已匹配，则匹配 true-pattern，否则匹配 false-pattern。 |
